 ...
 *
 * Symmetric inverse of RdmaInteropD3D11Dvp on the output side:
 *   - Capture thread: AJA DMAs SDI frame -> page-locked sysmem buffer
 *     (one of N pre-registered DVP buffers)
 *   - Capture thread: dvpMemcpyLined(sysmem -> texture) inside its own
 *     dvpBegin/dvpEnd
 *   - Render thread: dvpMapBufferWaitAPI(texture) inserts a wait into
 *     the D3D11 immediate context queue -> samples texture -> EndAPI
 *
 * The output texture's pixel layout matches the AJA byte layout
 * (UYVY -> RGBA8 at width/2 * height; ARGB/RGBA -> 4-byte at width *
 * height). DVP doesn't care about the texture's logical format - it
 * DMAs raw bytes - so any QRhi-allocated texture whose byte size
 * matches works. Downstream YUV->RGB conversion (for YCbCr8) is the
 * AJAInputNode's responsibility, not the strategy's.
 */
struct CaptureInteropD3D11Dvp final : score::gfx::interop::GpuDirectCaptureStrategy
{
  ...
  bool init(const score::gfx::interop::GpuDirectCaptureStrategyConfig& c) override
  {
    ...
    qDebug() << "AJA DVP-IN(D3D11): loading dvp.dll...";
    if(nv_dvp_init_d3d11(&m_dvpCtx, m_dev) != NV_DVP_SUCCESS || !m_dvpCtx)
    {
      qWarning() << "AJA DVP-IN(D3D11): init failed:"
                 << nv_dvp_get_error_string(m_dvpCtx);
      return false;
    }
    if(nv_dvp_thread_begin(m_dvpCtx) != NV_DVP_SUCCESS)
    {
      qWarning() << "AJA DVP-IN(D3D11): thread_begin failed:"
                 << nv_dvp_get_error_string(m_dvpCtx);
      release();
      return false;
    }
    m_threadStarted = true;
    qDebug() << "AJA DVP-IN(D3D11): dvp.dll loaded + D3D11 device bound";

    // Geometry from caller's QRhi texture; sysmem stride/size derived
    // and validated against the AJA frame byte size.
    ...
    {
      qWarning() << "AJA DVP-IN(D3D11): texture byte size" << m_sysmemBytes
                 << "(" << m_texW << "x" << m_texH << " RGBA8/BGRA8) !="
                 << "AJA frame size" << cfg.frameByteSize
    ...
    if(!nt.object)
    {
      qWarning() << "AJA DVP-IN(D3D11): output texture has no native handle";
      release();
      return false;
    }
    ...
    if(nv_dvp_register_d3d11_texture(
       ...
       != NV_DVP_SUCCESS)
    {
      qWarning() << "AJA DVP-IN(D3D11): register_d3d11_texture failed:"
                 << nv_dvp_get_error_string(m_dvpCtx);
      release();
      return false;
    }

    // Sysmem slots: page-locked for AJA's DMA + DVP-registered for
    // GPU DMA. AJA's frame size (cfg.frameByteSize) is what gets
    // DMA'd in by AutoCirculateTransfer; the DVP transfer copies
    // the same byte count out to the texture.
    for(auto& slot : m_slots)
    {
      slot.sysmem = nv_dvp_aligned_alloc(m_sysmemBytes);
      if(!slot.sysmem)
      {
        qWarning() << "AJA DVP-IN(D3D11): nv_dvp_aligned_alloc(" << m_sysmemBytes
                   << ") failed";
        release();
        return false;
      }
      ...
      if(!m_card->DMABufferLock(
             reinterpret_cast<const ULWord*>(slot.sysmem),
             static_cast<ULWord>(m_sysmemBytes), /*inMap=*/true,
             /*inRDMA=*/false))
      {
        qWarning() << "AJA DVP-IN(D3D11): DMABufferLock(slot) failed";
        release();
        return false;
      }
      slot.dmaLocked = true;
      if(nv_dvp_register_sysmem_buffer(
         ...
         != NV_DVP_SUCCESS)
      {
        qWarning() << "AJA DVP-IN(D3D11): register_sysmem_buffer failed:"
                   << nv_dvp_get_error_string(m_dvpCtx);
        release();
        return false;
      }
  ...
  void release() override
  {
    if(m_dvpCtx)
    {
      for(auto& slot : m_slots)
      {
        ...
        if(slot.dmaLocked && m_card && slot.sysmem)
        {
  ...
  bool ingestFrame(std::size_t i) override
  {
    ...
    if(nv_dvp_copy_buffer_to_texture(m_dvpCtx, slot.dvpBuf, m_dvpTex)
       != NV_DVP_SUCCESS)
    {
      qWarning() << "AJA DVP-IN(D3D11): copy_buffer_to_texture failed:"
                 << nv_dvp_get_error_string(m_dvpCtx);
      return false;
    }
    return true;
  ...
  void acquireForRender() override
  {
    if(m_dvpCtx && m_dvpTex)
    {
      ...
      {
        qWarning() << "AJA DVP-IN(D3D11): acquire_texture failed:"
                   << nv_dvp_get_error_string(m_dvpCtx);
      }
    }
  }
  ...
  void releaseAfterRender() override
  {
    if(m_dvpCtx && m_dvpTex)
    {
      ...
      {
        qWarning() << "AJA DVP-IN(D3D11): release_texture failed:"
                   << nv_dvp_get_error_string(m_dvpCtx);
      }
    }
  }
